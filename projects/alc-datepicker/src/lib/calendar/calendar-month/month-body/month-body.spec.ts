...
import { buildMonth } from './month-body';
import { WeekDay } from '@angular/common';

const testBuiltMonth = (
  builtMonth: Week[],
  month: CalendarDate,
  expectedMonthDays: number[][]
) => {
...
const testDates = (week: Week, month: CalendarDate) =>
  week.forEach((day) => {
    const dateInBuiltWeek = new CalendarDate(
...
const testIDs = (week: Week, month: CalendarDate) =>
  week.forEach((day) => {
    const idOfDayInBuiltWeek = new CalendarDate(
      month.getUTCFullYear(),
      month.getUTCMonth(),
      day.dayNumber
    )
...
describe('buildMonth', () => {
  it('should produce a month that has a full first week', () => {
    ...
    const builtMonth = buildMonth(month);
